...
|[sctp-bdc-v-7-4](sctp-bdc-v-7-4.pkt) | Yes (Note 3)| Yes       | No         | Passed         | Passed         |
...
|[sctp-bdc-v-7-8](sctp-bdc-v-7-8.pkt) | Yes (Note 4)| Yes       | No         | Passed         | Failed (Note 5)|
...
2. FreeBSD just ignores the packet containing the INIT-ACK chunk bundled with a DATA chunk whereas Linux responds with an ABORT indicating a protocol violation.
3. It is assumed that the SUT bundled the COOKIE-ECHO chunk with the SACK chunk.
4. It is assused that the SUT ignores the DATA chunk, which is bundled with the SHUTDOWN-ACK chunk, and sends a SHUTDOWN-COMPLETE chunk.
5. The message flow is correct. The problem is read() not returning 0 after the teardown handshake is complete.
